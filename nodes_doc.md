...
Utility node to process an image with longest edge scaling and padding, using the same logic as EditTextEncode without CLIP/VAE encoding.

##### Inputs
- **image**: The input image to process
- **ref_longest_edge**: Target longest edge size (default: 1024, range: 8-4096)
- **ref_crop**: Crop method - "pad", "center", or "disabled" (default: "pad")
- **ref_upscale**: Upscale method - "lanczos", "bicubic", or "area" (default: "lanczos")
- **vae_unit**: VAE unit size for padding alignment (default: 8, range: 8-64, step: 8)

##### Outputs
- **processed_image**: The resized and padded image
- **pad_info**: Padding information dictionary (compatible with CropWithPadInfo)
- **scale_by**: The scale factor for later use with CropWithPadInfo

##### Behavior
- Extracts the same longest-edge scaling and padding logic from EditTextEncode
- When ref_crop is "pad", pads the image to the nearest vae_unit multiple
- When ref_crop is "center" or "disabled", rounds dimensions to nearest vae_unit multiple
- Returns pad_info for use with CropWithPadInfo to restore original dimensions

#### ClearRefLatents_EditUtils
Utility node to strip reference latents from a conditioning tensor.

##### Inputs
- **conditioning**: The conditioning tensor with reference latents

##### Outputs
- **conditioning**: The same conditioning with reference latents cleared

##### Behavior
- Removes all reference latents from the conditioning
- Preserves all other conditioning properties
- Useful for reusing text encoding without image references

#### SaveCondition_EditUtils
Utility node to save a conditioning tensor to a `.ckpt` file.

##### Inputs
- **condition**: The conditioning tensor to save
- **filename**: Output filename (default: "condition_tensor", automatically adds .ckpt extension)

##### Outputs
- None (output node)

##### Behavior
- Saves conditioning to `models/conditions/` directory
- Creates the directory if it doesn't exist
- Sanitizes filename to prevent path traversal

#### LoadCondition_EditUtils
Utility node to load a conditioning tensor from a `.ckpt` file.

##### Inputs
- **filename**: Dropdown selection of available `.ckpt` files in the conditions directory

##### Outputs
- **conditioning**: The loaded conditioning tensor

##### Behavior
- Loads conditioning from `models/conditions/` directory
- Provides dropdown of available files
- Returns empty list on error as fallback

#### LoadConditionFromLoras_EditUtils
Utility node that lists files from the loras directory and loads matching `.ckpt` condition files.

##### Inputs
- **filename**: Dropdown selection of available lora filenames

##### Outputs
- **conditioning**: The loaded conditioning tensor

##### Behavior
- Lists files from loras directory as dropdown options
- Attempts to load matching `.ckpt` from `models/conditions/` directory
- Returns empty list on error as fallback
